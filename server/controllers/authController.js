...
export async function register(req,res) {
  try {
    ...
    // Fix: sameSite 'none' & secure true for Vercel <-> Render cross-domain authentication
    res.cookie('access_token',issue(u),{
      httpOnly:true,
      sameSite:'none',
      secure:true,
    ...
    console.error('Registration Exact Error:', e);
    res.status(500).json({
      error:'Failed to create account.',
      details: e.sqlMessage || e.message || 'Database error'
    });
  }
}
...
export async function login(req,res) {
  try {
    ...
    console.error('Login Exact Error:', e);
    res.status(500).json({
      error:'Login failed.',
      details: e.sqlMessage || e.message || 'Database error'
    });
